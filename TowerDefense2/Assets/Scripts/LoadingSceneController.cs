...
public class LoadingSceneController : MonoBehaviour
{
    ...
    private IEnumerator LoadingDisplayRoutine()
    {
        ...
        for(float t = 0; t < LoadingScene.FadeTime; t += Time.unscaledDeltaTime)
        {
            leftClosePart.rectTransform.anchoredPosition = 
                Vector2.Lerp(lStart, Vector2.zero, quadOut.Evaluate(t / LoadingScene.FadeTime));

            rightClosePart.rectTransform.anchoredPosition = 
                Vector2.Lerp(rStart, Vector2.zero, quadOut.Evaluate(t / LoadingScene.FadeTime));
        ...
        LoadingScene.EndLoading = true;

        for(float t = 0; t < LoadingScene.FadeTime; t += Time.unscaledDeltaTime)
        {
            leftClosePart.rectTransform.anchoredPosition = 
                Vector2.Lerp(Vector2.zero, lStart, quadIn.Evaluate(t / LoadingScene.FadeTime));

            rightClosePart.rectTransform.anchoredPosition = 
                Vector2.Lerp(Vector2.zero, rStart, quadIn.Evaluate(t / LoadingScene.FadeTime));
            yield return unscaledWait;
        }
    }
